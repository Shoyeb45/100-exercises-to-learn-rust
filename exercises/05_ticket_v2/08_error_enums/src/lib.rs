...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Err(err) => {
            match err {
                TicketNewError::DescriptionError(err_message) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
                TicketNewError::TitleError(err_message) => panic!("{err_message}")
            }
        }
    }
}
