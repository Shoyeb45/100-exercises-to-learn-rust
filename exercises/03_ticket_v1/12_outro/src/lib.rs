...
impl Order {
    pub fn validate_product_name(product_name: &String) {
        if product_name.is_empty() {
            panic!("Please provide product name");
        }
        if product_name.len() > 300 {
            panic!("Please provide shorter name")
        }
    }

    pub fn validate_quantity(quantity: &i32) {
        if *quantity <= 0 {
            panic!("Please provide non-zero quantity")
        }
    }

    pub fn validate_unit_price(unit_price: &i32) {
        if *unit_price <= 0 {
            panic!("Please provide non zero unit price");
        }
    }
